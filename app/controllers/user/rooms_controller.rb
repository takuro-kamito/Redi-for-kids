class User::RoomsController < ApplicationController
  ...
  def create
    ...
    redirect_to rooms_path(@room) #@another_entry変数に、`params[:entry][:user_id]`で指定されたユーザーのエントリーを作成。`params[:entry][:user_id]`は、リクエストパラメータの`entry`ハッシュの`user_id`キーの値を取得しています。このエントリーには、`user_id`として指定されたユーザーのID、`room_id`として先ほど作成したチャットルームのIDを指定
  end
